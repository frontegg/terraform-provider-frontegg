...
func TestWebhookHeadersRequireAVendorID(t *testing.T) {
	if _, err := fronteggWebhookHeaders(&restclient.ClientHolder{}); err == nil {
		t.Fatal("expected an error when no vendor ID is available, got nil")
	}
}

func TestWebhookHeadersSendVendorIDAsTenant(t *testing.T) {
	headers, err := fronteggWebhookHeaders(&restclient.ClientHolder{VendorID: "vend-1"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got := headers.Get("frontegg-tenant-id"); got != "vend-1" {
		t.Errorf("frontegg-tenant-id = %q, want %q", got, "vend-1")
	}
}
